...
## 追加分析（2026-08-23、司令塔依頼「単純なKPI数だけで優位性を潰してよいか」への回答）

司令塔から「月次シャープ・トレード数は向上しているのに、KPI数だけで判定してよいか」との指摘を受け、mean_r_net(0.357→0.301)の低下・permutation_p非有意化の**原因**を、85件のトレンドイベント単位でM5版・M3版を突き合わせて特定した(`scripts/analyze_m3_vs_m5_entry_opportunities.py`)。**正式な判定基準を追加・変更するものではない、原因分析**。

**機会面**: M5が完全に見逃した機会をM3が拾ったケースは0件。M3が完全に見逃したケースが1件。「M5では拾えない全く新しい機会」という効果は実質ゼロ。M3が増やした90トレードは、既存85イベント内の**追加の押し目(ピラミッディング的な再エントリー)**によるもの(37/85イベントでM3の方がトレード数が多い)。

**収益面**（合計r_net差分+10.23Rの内訳）:

| グループ | イベント数 | r_net差分(M3-M5) |
|---|---|---|
| M3の方がトレード数が多い(追加の押し目) | 37件 | **-10.81**(悪化) |
| M3の方がトレード数が少ない | 8件 | +3.59(改善) |
| トレード数が同じ(同一シグナル、執行タイミングのみ差) | 40件 | **+17.45**(改善) |

合計+10.23Rの改善は、「同一シグナルに対する執行精度の向上」(+17.45R、本物の優位性)が「トレンド継続中の追加ピラミッディング」(-10.81R、正味の足かせ)で一部相殺された結果。mean_r_netの低下とpermutation_pの非有意化は、この追加ピラミッディング分がノイズ同然のトレードを大量に混入させたことが直接の原因と判明。詳細: `research/method-notes/m3_vs_m5_entry_opportunities.json`

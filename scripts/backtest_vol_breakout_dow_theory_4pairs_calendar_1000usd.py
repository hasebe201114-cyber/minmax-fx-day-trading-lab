"""EXP-FX000005 改善ループ第3試行(4通貨版+経済指標カレンダーのブラックアウト窓)の
Train/Validation/Test 3期間・$1,000初期資金・確定コストモデルバックテスト.

背景: 司令塔依頼で連敗パターンを分析した結果、Train/Test各期間の最大連敗
(8連敗)がいずれもBOJ金融政策決定会合の当日・前日に集中していたと判明
(Train: 2024-04-26会合当日、Test: 2026-07-30〜31会合前後にBOJ/財務省の
覆面介入観測)。Validationの6連敗(2025-06-13)はイスラエルのイラン攻撃という
予測不能な地政学ショックで、カレンダーでは防げないことも判明済み。

司令塔選択「①経済指標カレンダーと照合して主要イベント前後は新規エントリーを
停止する」を受け、`economic_calendar.py`(BOJ・FOMC会合日程、WebSearch検証済み、
[開催初日-24h, 最終日+24h]のブラックアウト窓)を`simulate_dow_theory_trend()`の
`blackout_check`引数経由で適用する。検出層・SL/TP構造・パラメータ
(`vol_breakout_dow_theory_4pairs_train.json`の導出値)はすべて4通貨版から
変更しない(カレンダーは外生データであり、結果を見てから調整するパラメータ
ではないためTrain単独での再導出は不要)。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_calendar_1000usd_backtest.json
"""
...
def main() -> int:
    print("=== SYS-FX011 ダウ理論連続押し目買い版(4通貨版+経済指標カレンダー) Train/Validation/Test $1,000バックテスト ===")
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_calendar_1000usd_backtest.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": (
                "改善ループ第3試行: 4通貨版にBOJ/FOMC会合ブラックアウト窓を追加。検出層・コストモデル・"
                "パラメータは4通貨版から変更しない(economic_calendar.pyの窓定義のみ追加)。"
            ),

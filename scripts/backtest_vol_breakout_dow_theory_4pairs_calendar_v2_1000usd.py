"""EXP-FX000005 改善ループ第4試行(カレンダー窓のみ単独): buffer=6h・BOJのみの
Train/Validation/Test $1,000バックテスト.

背景: 感度分析(analyze_vol_breakout_entry_sensitivity.py)のPart Aで検証した
N=2.5・zigzag=0.6への緩和は、実効サンプル数を大きく増やした一方で最大連続損失
(Train20/Validation9/Test10)・ペイオフ比(0.80〜0.90)・スプレッドコスト倍率
(2.44〜2.82)が明確に悪化し、正式KPI判定もTrain・Testで第3試行(5/10・6/10)より
悪化(4/10・3/10)したため不採用と判断した。

Part Bで単独検証したカレンダー窓(buffer=6h・BOJのみ)はTrainでの効果が穏やか
(trades 387→463, mean_R 0.4277→0.4076, n_eff 113.0→135.2)だったため、
N/ZigZagは第3試行の値(N=3.5, zigzag=1.0)のまま維持し、カレンダー窓のみを
単独で緩和した場合の効果を確認する。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_calendar_v2_1000usd_backtest.json
"""
...
BLACKOUT_BUFFER_HOURS_V2 = 6
BLACKOUT_MEETINGS_V2 = BOJ_MEETINGS
_blackout_check_v2 = make_blackout_check(buffer_hours=BLACKOUT_BUFFER_HOURS_V2, meetings=BLACKOUT_MEETINGS_V2)
...
def find_trades_for_period(pair: str, m5: pd.DataFrame) -> tuple[list[dict], pd.DataFrame, pd.Series]:
    ...
    for i in idxs:
        ...
        trades.extend(simulate_dow_theory_trend(m5, atr_m5, h1, atr_h1, pos, direction,
                                                  ...
                                                  blackout_check=_blackout_check_v2))
...
def main() -> int:
    print("=== SYS-FX011 改善ループ第4試行(カレンダー窓のみ: buffer=6h+BOJのみ) "
          "Train/Validation/Test $1,000バックテスト ===")
    print(f"対象通貨: {SELECTED_PAIRS}")
    print(f"事前登録: N={N_BREAKOUT}, zigzag_threshold_atr_m5={TRAIN_PARAMS['zigzag_threshold_atr_m5']}(第3試行から不変), "
          f"blackout_buffer_hours={BLACKOUT_BUFFER_HOURS_V2}, blackout_scope=BOJのみ")
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_calendar_v2_1000usd_backtest.json"
    out_path.write_text(
        json.dumps({
            ...
            "design": {
                ...
                "blackout_buffer_hours": BLACKOUT_BUFFER_HOURS_V2, "blackout_scope": "boj_only",
            ...
            "_note": (
                "改善ループ第4試行(カレンダー窓単独): N/ZigZagは第3試行の値(N=3.5,zigzag=1.0)を維持し、"
                "カレンダー窓のみbuffer=6h+BOJのみに緩和。N/ZigZag緩和(N=2.5/zigzag=0.6)は最大連続損失・"
                "ペイオフ比の悪化により別途不採用と判断済み(vol_breakout_dow_theory_4pairs_v4参照)。"
            ),

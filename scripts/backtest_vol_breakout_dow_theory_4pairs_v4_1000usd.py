"""EXP-FX000005 改善ループ第4試行: エントリー機会拡大版(N=2.5・ZigZag=0.6・
カレンダー窓6h+BOJのみ)のTrain/Validation/Test $1,000バックテスト.

背景: 4通貨+カレンダー版(改善ループ第3試行)でも実効サンプル数(min_n_trades=300)
未達が最大のボトルネックとして残っていた(Train113.0/Validation35.0/Test53.4)。
司令塔へ提示したエントリー機会拡大の4方策のうち、司令塔選択
「③カレンダー窓の縮小、①N閾値の緩和、②ZigZag閾値の緩和」を受け、
`analyze_vol_breakout_entry_sensitivity.py`でTrain(4通貨プール)のみを対象に
感度分析を実施した(HARKing防止、事前登録した選定ルール: トレード数が現行より
増加しmean_Rが正を維持する候補のうちn_trades_effectiveが最大のものを選定)。

選定結果:
- Part A(N x ZigZag、カレンダー無し): **N=2.5, zigzag_threshold_atr_m5=0.6**
  (trades 568→2621、mean_R 0.4735→0.5176、n_eff 165.9→765.3)
- Part B(カレンダー窓、N=3.5/zigzag=1.0固定): **buffer=6h, 対象会合=BOJのみ**
  (trades 387→463、mean_R 0.4277→0.4076、n_eff 113.0→135.2)

両者はそれぞれ独立したパートで別個に選定されたものであり、組み合わせての
評価は本スクリプトが初回である。stop_buffer_atr_m5(=0.703)・
atr_trail_multiplier(=3.23)はN/ZigZagに依存しない値であることを感度分析で
確認済みのため、4通貨版Train導出値をそのまま使用する(再導出不要)。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_v4_1000usd_backtest.json
"""
...
from economic_calendar import BOJ_MEETINGS, make_blackout_check  # noqa: E402
...
# 改善ループ第4試行: 感度分析で選定した値(事前登録済み選定ルールに基づく、結果を見てからの調整ではない)
N_BREAKOUT_V4 = 2.5
ZIGZAG_THRESHOLD_ATR_M5_V4 = 0.6
BLACKOUT_BUFFER_HOURS_V4 = 6
BLACKOUT_MEETINGS_V4 = BOJ_MEETINGS  # FOMCは除外(boj_onlyが選定された)
_blackout_check_v4 = make_blackout_check(buffer_hours=BLACKOUT_BUFFER_HOURS_V4, meetings=BLACKOUT_MEETINGS_V4)
...
def find_trades_for_period(pair: str, m5: pd.DataFrame) -> tuple[list[dict], pd.DataFrame, pd.Series]:
    ...
    idxs = np.where(ratio.values >= N_BREAKOUT_V4)[0]
    ...
    for i in idxs:
        ...
        trades.extend(simulate_dow_theory_trend(m5, atr_m5, h1, atr_h1, pos, direction,
                                                  ...
                                                  blackout_check=_blackout_check_v4,
                                                  zigzag_threshold_atr_m5=ZIGZAG_THRESHOLD_ATR_M5_V4))
...
def run_period(period_name: str, start: str, end: str) -> dict:
    ...
    peak_running = np.maximum.accumulate([b if b > 0 else 1e-9 for b in balances])
    peak_dd = [(b - m) / m * 100.0 for b, m in zip(balances, peak_running)]
    peak_max_dd_pct = min(peak_dd) if peak_dd else 0.0
    ...
    print(f"  トレード数={n} (破産ガードでスキップ={n - n_effective_trades})  最終残高=${final_balance:.2f}  "
          f"総リターン={total_return_pct:.1f}%  最大DD(初期資金比)={max_dd_pct:.1f}%  最大DD(ピーク比)={peak_max_dd_pct:.1f}%")
    ...
    return {
        ...
        "max_drawdown_peak_relative_pct": round(peak_max_dd_pct, 2),
...
def main() -> int:
    print("=== SYS-FX011 改善ループ第4試行(N=2.5・ZigZag=0.6・カレンダー6h+BOJのみ) "
          "Train/Validation/Test $1,000バックテスト ===")
    print(f"対象通貨: {SELECTED_PAIRS}")
    print(f"事前登録(感度分析で選定): N={N_BREAKOUT_V4}, zigzag_threshold_atr_m5={ZIGZAG_THRESHOLD_ATR_M5_V4}, "
          f"blackout_buffer_hours={BLACKOUT_BUFFER_HOURS_V4}, blackout_scope=BOJのみ, "
          f"stop_buffer_atr_m5={STOP_BUFFER_ATR_M5}(4通貨版から不変), atr_trail_multiplier={ATR_TRAIL_MULTIPLIER}(不変)")
    ...
    print(f"\n=== サマリ ===")
    print(f"{'期間':<12}{'取引数':>8}{'最終残高':>12}{'総リターン':>10}{'最大DD(peak)':>12}{'勝率':>8}{'PF':>8}{'perm_p':>8}")
    for name, r in period_results.items():
        print(f"{name:<12}{r['n_trades']:>8}{'$'+str(r['final_balance_usd']):>12}"
              f"{str(r['total_return_pct'])+'%':>10}{str(r['max_drawdown_peak_relative_pct'])+'%':>12}"
              f"{r['win_rate']:>8}{r['profit_factor']:>8}{r['perm_p_clustered']:>8}")

    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_v4_1000usd_backtest.json"
    out_path.write_text(
        json.dumps({
            "generated_at": datetime.now().isoformat(),
            "selected_pairs": SELECTED_PAIRS,
            "design": {
                "n_breakout": N_BREAKOUT_V4, "zigzag_threshold_atr_m5": ZIGZAG_THRESHOLD_ATR_M5_V4,
                "blackout_buffer_hours": BLACKOUT_BUFFER_HOURS_V4, "blackout_scope": "boj_only",
            ...
            "_note": (
                "改善ループ第4試行: analyze_vol_breakout_entry_sensitivity.pyのTrain感度分析(Part A: N/ZigZag、"
                "Part B: カレンダー窓)で個別に選定した値を組み合わせて初めて評価する。HARKing防止のため選定は"
                "Trainのみに基づく。組み合わせでの評価は本スクリプトが初回。"
            ),

"""EXP-FX000005 T-13: 出口設計をトレール専業に振る(司令塔決定、2026-08-21).

外部レビュー§1-1・C査読が指摘した通り、段階利確(TP1/TP2/TP3)+建値化を維持した
ままATRトレーリングをM5スケールに修正しても(T-02)、ペイオフレシオはむしろ
全期間で1.0を大きく下回る水準まで悪化した(Train/Validation/Test:
0.79/0.77/0.74)。K4m(ペイオフ≥1.5)に近づくための2つの選択肢
((a)トレール専業に振る、(b)K4mを非適用と宣言する)について司令塔に確認した
結果、**(a)を選択**。

事前登録(00-spec.md「T-13」節、結果を見る前に固定):
  - `tp_levels = []`(段階利確を全廃、ポジション全量をトレーリングストップのみ
    で管理)
  - `breakeven_trigger_r = 1.0`(含み益が+1.0Rに達した時点で建値へ移動。新規
    パラメータではなく、旧TP1水準(1.0R)を「建値移行の唯一のトリガー」として
    転用したもの)
  - ATRトレーリング倍率はT-02で確立済みの値(`atr_trail_multiplier_m5 =
    stop_buffer_atr_m5 × 1.0`)をそのまま使用、変更しない

検出層・エントリー層(価格反応型ショック抑制フィルター含む)・コストモデル
(T-09)・検定方式(T-06/T-07)・重複トレード修正はすべて現行のまま変更しない。
Test期間は`00-spec.md`「Test凍結宣言」節により参照するが、結論・意思決定には
用いない(Train+Validationのみで判断する)。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_v7_trailonly_1000usd_backtest.json
"""
...
def main() -> int:
    print("=== SYS-FX011 T-13: 出口設計をトレール専業に振る(T-01〜T-09累積+dedup+trailonly) ===")
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_v7_trailonly_1000usd_backtest.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": (
                "T-01+T-02+T-03+T-06+T-07+T-09+重複トレード生成バグ修正(dedup)に、"
                "T-13(出口設計をトレール専業に振る: tp_levels=[]で段階利確を全廃し、"
                "breakeven_trigger_r=1.0(旧TP1水準を建値移行の唯一のトリガーに転用)で"
                "ポジション全量をATRトレーリングのみで管理)を追加適用。perm_p_block(新)と"
                "perm_p_clustered(旧、比較用)を両方記録する。Testも参照するが、"
                "00-spec.md「Test凍結宣言」によりTrain+Validationのみで意思決定する。"
            ),

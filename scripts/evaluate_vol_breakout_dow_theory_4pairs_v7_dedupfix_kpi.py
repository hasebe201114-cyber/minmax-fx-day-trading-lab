"""EXP-FX000005 重複トレード生成バグ修正後の正式KPI判定.
...
で呼び出し、対象をT-01+T-02+T-03+T-06+T-07+T-09+重複バグ修正の$1,000バックテスト
結果に差し替える。

00-spec.md「Test凍結宣言」により、Testの数値もここで算出・記録するが、
修正の妥当性確認のみを目的とし、結論・意思決定にはTrain+Validationのみを用いる。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_v7_dedupfix_kpi_evaluation.json
"""
...
def main() -> int:
    print("=== EXP-FX000005 重複トレード生成バグ修正後 正式KPI判定 ===\n")
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_v7_dedupfix_kpi_evaluation.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": (
                "T-01+T-02+T-03+T-06+T-07+T-09に加え、重複トレード生成バグの修正"
                "(select_non_overlapping_breakout_events())を適用した正式KPI判定。"

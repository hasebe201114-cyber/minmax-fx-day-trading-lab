"""EXP-FX000017(SYS-FX023) 改善ループ第1試行: BOJ+FOMC窓での通貨拡大
(7通貨 候補A vs 6通貨 候補B、Train単独).

事前登録(`research/EXP-FX000017/00-spec.md`「改善ループ第1試行」節):
既存4通貨(USD/JPY・EUR/JPY・GBP/JPY・AUD/JPY)に、SYS-FX016(EXP-FX000010)で
既に取得済み・スプレッド確定済みのNZD/CAD/CHF_JPYを追加する。窓定義は
Trainベースライン評価の候補①(BOJ+FOMC窓)に固定(1変数=通貨構成のみを動かす)。

候補A(7通貨) = 既存4通貨+NZD/CAD/CHF_JPY
候補B(6通貨) = 既存4通貨+CAD/CHF_JPY(NZD_JPYを除外、SYS-FX016の既存知見をそのまま適用)

出力: research/method-notes/sysfx023_currency_expansion_trainonly_backtest.json
"""
...
from economic_calendar import build_blackout_windows  # noqa: E402
...
# SYS-FX016(EXP-FX000010)確定値をそのまま流用、本試行用の新規スプレッド仮定なし
NEW_PAIR_SPREAD_PIPS = {"NZD_JPY": 1.2, "CAD_JPY": 1.0, "CHF_JPY": 1.5}
ALL_SPREAD_PIPS = {**SPREAD_PIPS, **NEW_PAIR_SPREAD_PIPS}

CANDIDATES = {
    "candidateA_7pairs": ["USD_JPY", "EUR_JPY", "GBP_JPY", "AUD_JPY", "NZD_JPY", "CAD_JPY", "CHF_JPY"],
    "candidateB_6pairs_no_nzd": ["USD_JPY", "EUR_JPY", "GBP_JPY", "AUD_JPY", "CAD_JPY", "CHF_JPY"],
}
...
def run_period(pairs: list[str], windows, start: str, end: str) -> dict:
...
def main() -> int:
    ...
    windows = build_blackout_windows()  # 候補①(BOJ+FOMC窓)に固定
    print("=== EXP-FX000017(SYS-FX023) 改善ループ第1試行: 通貨拡大 (Train単独) ===\n")

    results = {}
    kpis = {}
    for name, pairs in CANDIDATES.items():
        print(f"--- {name} ({len(pairs)}通貨: {pairs}) ---")
        result = run_period(pairs, windows, start, end)
    ...
    baseline_kpi_count = 5  # Trainベースライン評価(候補①②とも5/9)
    ...
    if eligible:
        ...
        best_count = int(best[1]["kpi_required_pass_count"].split("/")[0])
        selected = best if best_count >= baseline_kpi_count else None
        diluted = best_count < baseline_kpi_count
    else:
        selected = None
        diluted = False

    print("=== サマリ ===")
    print(f"{'candidate':<28}{'n_trades':>10}{'ペイオフ':>10}{'KPI':>8}{'実効n':>8}{'perm_p':>10}")
    for name in CANDIDATES:
        k = kpis[name]
        print(f"{name:<28}{results[name]['n_trades']:>10}{k['payoff_ratio']:>10}{k['kpi_required_pass_count']:>8}"
    ...
    if selected:
        ...
        print("\n[選定] 実効n>=300は達成した候補があるが、必須KPI達成数がベースライン(5/9)を"
              "下回るため不採用(SYS-FX016と同じ質の希薄化パターン)")
    else:
        print("\n[選定] 該当候補なし(実効n>=300を満たす候補が存在しない) → "
              "正式な採否判断は不可、実効n不足として記録")

    out_path = ROOT / "research" / "method-notes" / "sysfx023_currency_expansion_trainonly_backtest.json"
    out_path.write_text(json.dumps({
        "generated_at": pd.Timestamp.now().isoformat(),
        "purpose": "EXP-FX000017(SYS-FX023) 改善ループ第1試行: BOJ+FOMC窓での通貨拡大(7通貨 vs 6通貨)",
        "candidates": {k: v for k, v in CANDIDATES.items()},
        "baseline_kpi_count_for_comparison": baseline_kpi_count,

"""EXP-FX000005 重複トレード生成バグの修正(2026-08-21、司令塔指示で最優先対応).

T-12(証拠金維持率ストレステスト)の実施中に発見: `find_trades_for_period()`は
H1ブレイク検知バー(N_BREAKOUT以上)ごとに独立して`simulate_dow_theory_trend()`
を呼び出していたが、持続的な高ボラ局面では複数のH1バーが連続してN_BREAKOUT
以上を満たすことが多く、それぞれが独立した追跡チェーン(1通貨1ポジション制約は
チェーン内でのみ有効で、チェーンをまたいでは共有されない)を開始していた。
これらのチェーンが同一のM5構造ピボットに収束すると、エントリー価格・方向・
時刻・決済理由・r_grossまで完全に一致する「同一トレード」が複数回生成され、
それぞれが(成長し続ける)口座残高に対して個別にサイジング・複利計算されて
しまっていた(実測: 2026-07-29〜31にUSD/JPYで5本のH1ブレイクバーが連続発生し、
同一トレードが5回生成。改善ループ第7試行の時点で既に存在していたバグで、
Testでは総利益の36.4%が重複計上由来だった)。

修正: `backtest_vol_breakout_dow_theory.select_non_overlapping_breakout_events()`
を新設(同一方向のブレイクイベントが、直前の同一方向イベントの追跡窓
`break_time+72h`内で発生した場合はスキップする。異なる方向=真の反転は窓内でも
許可)。`find_trades_for_period()`でブレイク検知後にこのフィルタを適用してから
`simulate_dow_theory_trend()`を呼ぶよう変更。回帰テスト
`tests/test_dedup_breakout_events.py`(5件)を新設。

T-01(週末強制クローズ)・T-02(ATRトレール倍率M5再導出)・T-03(決済判定のM5化)・
T-06(permutation検定のブロック順列化)・T-07(実効n二重計上の解消)・T-09
(コストモデルのスリッページ復活)はこのスクリプトでも適用済み(共有モジュールの
...
出力: research/method-notes/vol_breakout_dow_theory_4pairs_v7_dedupfix_1000usd_backtest.json
"""
...
def main() -> int:
    print("=== SYS-FX011 重複トレード生成バグ修正後(T-01〜T-09累積+dedup) ===")
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_v7_dedupfix_1000usd_backtest.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": (
                "T-01+T-02+T-03+T-06+T-07+T-09に、重複トレード生成バグの修正"
                "(select_non_overlapping_breakout_events()、同一方向の連続H1ブレイクイベントを"
                "追跡窓72h以内で間引く)を追加適用。perm_p_block(新)とperm_p_clustered(旧、比較用)"
                "を両方記録する。Testも参照するが、00-spec.md「Test凍結宣言」によりTrain+Validation"
                "のみで意思決定する。"
            ),

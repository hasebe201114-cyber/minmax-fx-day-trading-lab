"""EXP-FX000005 外部レビューT-01+T-02+T-03修正後(=T-04)の正式KPI判定.

`evaluate_vol_breakout_dow_theory_kpi.py`(ピーク比DD採用済み)と同一のロジックを
再利用し、対象を改善ループ第7試行+T-01(週末クローズ)+T-02(ATRトレールM5再導出)+
T-03(決済のM5化)の$1,000バックテスト結果に差し替える。

00-spec.md「Test凍結宣言」により、Testの数値もここで算出・記録するが、
修正の妥当性確認のみを目的とし、結論・意思決定にはTrain+Validationのみを用いる。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_v7_t03fix_kpi_evaluation.json
"""
...
def main() -> int:
    print("=== EXP-FX000005 外部レビューT-01+T-02+T-03修正後(=T-04) 正式KPI判定 ===\n")

    with (ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_v7_t03fix_1000usd_backtest.json").open(
        encoding="utf-8"
    ) as f:
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_v7_t03fix_kpi_evaluation.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": (
                "外部レビューT-01(週末クローズ)+T-02(ATRトレールM5再導出)+T-03(決済のM5化)を"
                "第7試行に累積適用した後の正式KPI判定(=T-04)。Test凍結宣言によりTestの数値も"
                "算出するが、結論・意思決定にはTrain+Validationのみを用いる。"
            ),

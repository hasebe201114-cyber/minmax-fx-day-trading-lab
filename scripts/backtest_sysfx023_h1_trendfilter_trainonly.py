"""EXP-FX000017(SYS-FX023) 改善ループ第2試行: BOJ+FOMC窓/BOJ単独窓に
H1トレンド判定不能除外フィルターを追加(Train単独).

事前登録(`research/EXP-FX000017/00-spec.md`「改善ループ第2試行」節):
Trainベースライン評価の候補①(BOJ+FOMC窓)・候補②(BOJ単独窓)それぞれに、
SYS-FX012で確立済みのH1トレンド判定不能除外フィルター
(`h1_dow_trend_direction()`、zigzag threshold_atr_h1=2.0、新規パラメータなし)
を重ねる。通貨構成は既存4通貨のまま(第1試行の通貨拡大とは組み合わせない)。

候補C = 候補①(BOJ+FOMC窓) + トレンド判定不能除外フィルター
候補D = 候補②(BOJ単独窓) + トレンド判定不能除外フィルター

出力: research/method-notes/sysfx023_h1_trendfilter_trainonly_backtest.json
"""
...
CANDIDATES = {
    "candidateC_boj_fomc_trendfilter": None,
    "candidateD_boj_only_trendfilter": BOJ_MEETINGS,
}
...
def main() -> int:
    ...
    print("=== EXP-FX000017(SYS-FX023) 改善ループ第2試行: H1トレンド判定不能除外フィルター (Train単独) ===\n")
    ...
    print("=== サマリ ===")
    print(f"{'candidate':<32}{'n_trades':>10}{'ペイオフ':>10}{'KPI':>8}{'実効n':>8}{'perm_p':>10}")
    for name in CANDIDATES:
        k = kpis[name]
        print(f"{name:<32}{results[name]['n_trades']:>10}{k['payoff_ratio']:>10}{k['kpi_required_pass_count']:>8}"
    ...
    elif best_count > BASELINE_KPI_COUNT:
        print(f"\n[選定] 実効n>=300は未達だが、質はベースライン({BASELINE_KPI_COUNT}/9)を上回った候補あり。"
              "実効n不足のまま追加検討の材料として記録")
    else:
        print(f"\n[選定] 該当候補なし(実効n>=300未達、かつベースライン{BASELINE_KPI_COUNT}/9を上回らない) → 不採用")

    out_path = ROOT / "research" / "method-notes" / "sysfx023_h1_trendfilter_trainonly_backtest.json"
    out_path.write_text(json.dumps({
        "generated_at": pd.Timestamp.now().isoformat(),
        "purpose": "EXP-FX000017(SYS-FX023) 改善ループ第2試行: H1トレンド判定不能除外フィルター追加",

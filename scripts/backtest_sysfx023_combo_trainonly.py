"""EXP-FX000017(SYS-FX023) 改善ループ第4試行: N閾値緩和 × H1トレンド判定
不能除外フィルターの組み合わせ(Train単独).

事前登録(`research/EXP-FX000017/00-spec.md`「改善ループ第4試行」節):
第2試行(H1トレンド判定不能除外フィルター)・第3試行(N閾値緩和)でそれぞれ
個別検証済みの設定をそのまま組み合わせる(新規パラメータ探索なし)。
候補G = N=2.5+フィルター、候補H = N=3.0+フィルター。窓・通貨構成は不変。

出力: research/method-notes/sysfx023_combo_trainonly_backtest.json
"""
...
CANDIDATES = {
    "candidateG_n2.5_trendfilter": 2.5,
    "candidateH_n3.0_trendfilter": 3.0,
}

BASELINE_BEST_KPI_COUNT = 5  # ベースライン・第1〜3試行を通じた最良値
...
def find_trades(pair: str, m5: pd.DataFrame, windows, n_breakout: float) -> list[dict]:
    h1 = to_h1(m5)
...
def run_period(n_breakout: float, windows, start: str, end: str) -> dict:
    all_trades: list[dict] = []
    for pair in SELECTED_PAIRS:
        m5 = load_m5_period(pair, start, end)
        if len(m5) < 1000:
            continue
        trades = find_trades(pair, m5, windows, n_breakout)
...
def main() -> int:
    ...
    print("=== EXP-FX000017(SYS-FX023) 改善ループ第4試行: N緩和×トレンドフィルター組合せ (Train単独) ===\n")

    results = {}
    kpis = {}
    for name, n_breakout in CANDIDATES.items():
        print(f"--- {name} (N_BREAKOUT={n_breakout}) ---")
        result = run_period(n_breakout, windows, start, end)
    ...
    best_count = max(int(k["kpi_required_pass_count"].split("/")[0]) for k in kpis.values())
    if selected:
        print(f"\n[選定] {selected[0]} (KPI={selected[1]['kpi_required_pass_count']}、"
              f"ペイオフ={selected[1]['payoff_ratio']})")
    elif best_count > BASELINE_BEST_KPI_COUNT:
        print(f"\n[選定] 実効n>=300は未達だが、これまでの最良({BASELINE_BEST_KPI_COUNT}/9)を上回った候補あり")
    else:
        print(f"\n[選定] 該当候補なし(実効n>=300未達、これまでの最良{BASELINE_BEST_KPI_COUNT}/9も上回らない) "
              "→ SYS-FX019と同型の組み合わせ不採用パターン")

    out_path = ROOT / "research" / "method-notes" / "sysfx023_combo_trainonly_backtest.json"
    out_path.write_text(json.dumps({
        "generated_at": pd.Timestamp.now().isoformat(),
        "purpose": "EXP-FX000017(SYS-FX023) 改善ループ第4試行: N緩和×トレンドフィルター組合せ",
        "candidates": {k: v for k, v in CANDIDATES.items()},
        "baseline_best_kpi_count_for_comparison": BASELINE_BEST_KPI_COUNT,

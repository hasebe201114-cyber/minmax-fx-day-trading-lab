"""EXP-FX000005 外部レビュー対応T-01+T-02+T-03累積適用(=T-04の最終再計測).

外部レビュー§1-2「エントリーはM5時刻で決まるが、決済判定(`simulate_scaled_scheme`)は
H1バー単位(`entry_h1_idx+1`から)で回っており、エントリー直後〜そのH1バー終了まで
(平均約30分)のSL/TP到達が完全に不可視だった」への対応(T-03)。これをT-01(週末強制
クローズ修正)・T-02(ATRトレール倍率のM5再導出)に積み重ねた最終形。

T-03の変更: `simulate_dow_theory_trend(..., m5_exit=True)`を指定し、決済判定を
H1バー単位からM5バー単位に切り替える(トレール幅の計算方法自体はT-02から変更なし、
判定タイミングの粒度のみ変更)。

事前登録(T-01〜T-03すべて、結果を見る前に固定): T-01はISO週番号比較への変更、T-02は
トレール幅=1R×1.0(ATR(M5)基準)、T-03は決済判定をM5バー単位に変更。3つとも数値的な
根拠(週末バー不在の構造的欠陥/1Rとの定義整合/エントリー直後の不可視窓)から導出した
もので、結果を見てからの後付け調整ではない。

検出層・エントリー層・TP水準(TP3=4R)・価格反応型ショック抑制フィルターは第7試行から
完全に同一。Test期間は`00-spec.md`「Test凍結宣言」節により参照するが、修正の妥当性
確認のみを目的とし、結論・意思決定には用いない(Train+Validationのみで判断する)。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_v7_t03fix_1000usd_backtest.json
"""
...
def main() -> int:
    print("=== SYS-FX011 外部レビュー対応T-01+T-02+T-03累積適用(=T-04最終再計測) ===")
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_v7_t03fix_1000usd_backtest.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": (
                "外部レビューT-01(週末クローズ)+T-02(ATRトレールM5再導出、F3対応)+"
                "T-03(決済判定のM5化、§1-2対応)を第7試行(価格反応型ショック抑制フィルター、"
                "TP3=4R)に累積適用した最終形(=T-04の再計測結果)。Testも参照するが、"

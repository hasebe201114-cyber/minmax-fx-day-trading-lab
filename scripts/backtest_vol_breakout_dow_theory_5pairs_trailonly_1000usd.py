"""EXP-FX000005 T-14: 通貨ペア拡大(EUR_USD追加、5通貨化、2026-08-21).

外部レビュー・C査読が共通して指摘したT-14(通貨ペア拡大)への対応。通貨別均質性
の再検証(T-16再査読後)で、Validation・Testとも既存4通貨(すべてJPYクロス)の
うち1通貨が期間ごとに入れ替わりながら負に転じる不安定な集中依存が判明しており、
4通貨いずれもJPYクロスで相互相関が高い(実測: JPYクロス4通貨の平均相関0.808、
実効独立数1.17)ことが根本原因と考えられる。既存の相関分析(`market_character.json`)
では**USD/JPY-EUR/USDのみが-0.443という負の相関を持ち、唯一分散に寄与する
通貨ペア**と判明していた。

DS-1(`data/curated/ds-1.json`)には既にEUR_USDのデータが含まれており(新規API
取得は不要)、コストモデル(`SPREAD_PIPS`)・pip換算(`pip_size()`)ともEUR_USD
向けの設定が既に存在していた(トレール専業化のT-13時点では未使用のまま
残されていた)。本スクリプトは4通貨(USD/JPY・EUR/JPY・GBP/JPY・AUD/JPY)構成に
EUR_USDを追加した5通貨版として、T-01〜T-13すべてを不変のまま再評価する。

事前登録(結果を見る前に固定):
  - パラメータ(`stop_buffer_atr_m5`・`zigzag_threshold_atr_m5`・
    `atr_trail_multiplier_m5`)は4通貨Train導出値をそのまま流用し、EUR_USD
    追加を理由に再導出しない(ATR正規化済みの無次元比率であり、既存4通貨
    (JPYクロス3種+AUD/JPY)への適用時と同じ扱いを踏襲するのみで、新たな
    恣意的パラメータの追加ではない)。
  - 価格反応型ショック抑制フィルター(`price_shock_filter.py`)の
    `SIMULTANEOUS_PAIRS_REQUIRED=2`は絶対件数のまま変更しない(通貨数に対する
    比率ではなく「2通貨以上」という既存の絶対基準を5通貨でもそのまま適用)。
  - 検出層・エントリー層・コストモデル(T-09)・検定方式(T-06/T-07)・重複
    トレード修正・出口設計(T-13)・K3m定義(T-08)はすべて現行のまま変更しない。

Test期間は`00-spec.md`「Test凍結宣言」節により参照するが、結論・意思決定には
用いない(Train+Validationのみで判断する)。

出力: research/method-notes/vol_breakout_dow_theory_5pairs_trailonly_1000usd_backtest.json
"""
...
SELECTED_PAIRS = ["USD_JPY", "EUR_JPY", "GBP_JPY", "AUD_JPY", "EUR_USD"]  # T-14: EUR_USDを追加(4通貨→5通貨)
...
TP_LEVELS_V5 = [(1.0, 0.40), (2.0, 0.35), (4.0, 0.25)]  # 旧設計(比較用に定数のみ保持、本スクリプトでは不使用)
TP_LEVELS_TRAILONLY: list[tuple[float, float]] = []  # T-13: 段階利確を全廃、トレーリングのみで管理
...
def find_trades_for_period(pair: str, m5: pd.DataFrame, shock_check) -> tuple[list[dict], pd.DataFrame, pd.Series]:
    ...
    # 重複トレード生成バグの修正: 同一方向の連続ブレイクイベントを間引く
...
def main() -> int:
    print("=== SYS-FX011 T-14: 通貨ペア拡大(EUR_USD追加、5通貨、T-01〜T-13累積) ===")
    print(f"対象通貨: {SELECTED_PAIRS}")
    print(f"事前登録(4通貨Train導出値をそのまま使用、EUR_USD追加を理由に再導出しない): N={N_BREAKOUT}, "
          ...
          f"tp_levels={TP_LEVELS_TRAILONLY}(段階利確廃止), breakeven_trigger_r={BREAKEVEN_TRIGGER_R}, "
          f"filter=price_shock_reactive(trigger=7.0,calm=2.0,calm_bars=3), "
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_5pairs_trailonly_1000usd_backtest.json"
    out_path.write_text(
        json.dumps({
            ...
            "design": {
                ...
                "tp_levels": TP_LEVELS, "breakeven_trigger_r": BREAKEVEN_TRIGGER_R,
                ...
                "dedup_breakout_events": True,
                "exit_design": "trailonly(T-13)",
                "currency_expansion": "5pairs(T-14, +EUR_USD)",
            },
            "periods": period_results,
            "_note": (
                "T-01+T-02+T-03+T-06+T-07+T-09+重複トレード生成バグ修正(dedup)+T-13"
                "(出口設計をトレール専業に振る)に、T-14(通貨ペア拡大: EUR_USDを追加し"
                "4通貨→5通貨)を追加適用。パラメータは4通貨Train導出値をそのまま流用し、"
                "EUR_USD追加を理由に再導出していない。perm_p_block(新)とperm_p_clustered"
                "(旧、比較用)を両方記録する。Testも参照するが、00-spec.md「Test凍結宣言」に"
                "よりTrain+Validationのみで意思決定する。"
            ),

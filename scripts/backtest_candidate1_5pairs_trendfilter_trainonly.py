"""SYS-FX012改善ループ第4試行(改善ループ上限5回のうち最終回、`PJ000004`
「Q10」): 最良候補「N_BREAKOUT単独+H1トレンド判定不能除外フィルター」に
EUR_USDを追加した5通貨版のTrainベースライン評価.

背景: 改善ループ第1〜3試行を通じ、①(N_BREAKOUT単独+判定不能除外
フィルター)がTrain7/9・Validation6/9(合計13/18)で最良のまま、実効n=85
という脆弱性のみが未解決の課題として残った。司令塔の指示「通貨拡大」を
受け、既存4通貨(USD/JPY・EUR/JPY・GBP/JPY・AUD/JPY)にEUR_USD(DS-1に
既存データがあり新規取得不要)を追加した5通貨版で、nの底上げを試みる。

事前登録: 検出層(N_BREAKOUT)・H1トレンド判定不能除外フィルター・M5層
(エントリー/出口/コスト/検定)は一切変更しない。変えるのは対象通貨のみ。

注意(正直な記録): SYS-FX011のT-14では同じEUR_USD追加が明確な負の結果
(EUR_USD単体がTrainでマイナス、プール全体を希釈)だったが、それは
トレンドフィルターなしの旧設計での結果であり、本試行(判定不能除外
フィルター適用後)での結果は未確認のため独立に検証する。

出力: research/method-notes/candidate1_5pairs_trendfilter_trainonly_backtest.json
"""
...
SELECTED_PAIRS_5 = ["USD_JPY", "EUR_JPY", "GBP_JPY", "AUD_JPY", "EUR_USD"]


def run_period_5pairs(start: str, end: str) -> dict:
    print(f"\n=== train (5通貨): {start} 〜 {end} ===")
    m5_by_pair, h1_by_pair, atr_h1_by_pair = {}, {}, {}
    for pair in SELECTED_PAIRS_5:
    ...
    for pair, m5 in m5_by_pair.items():
        ...
        per_pair_n[pair] = {"n_events_raw": n_raw, "n_events_dedup": n_dedup,
                             "n_events_trendfiltered": n_trendfiltered, "n_trades": len(trades)}
        ...
        print(f"  [{pair}] イベント={n_raw}件(dedup後{n_dedup}、判定不能除外後{n_trendfiltered})  トレード={len(trades)}件")
    ...
    print(f"\nイベント総数={n_raw_total}(dedup後{n_dedup_total}、判定不能除外後{n_trendfiltered_total})  トレード数={n}")
...
def main() -> int:
    ...
    print("=== SYS-FX012改善ループ第4試行(最終) 候補①+判定不能除外フィルター 5通貨版(EUR_USD追加) ===")
    result = run_period_5pairs(start, end)
    ...
    # EUR_USD単体の寄与を確認(T-14と同じ懸念の再検証)
    eur_usd_trades = [t for t in result["trades"] if t["pair"] == "EUR_USD"]
    if eur_usd_trades:
        r = np.array([t["r_net"] for t in eur_usd_trades])
        print(f"\nEUR_USD単体: n={len(eur_usd_trades)}  mean_r_net={r.mean():.4f}  win_rate={(r>0).mean():.4f}")

    out = {
        "generated_at": pd.Timestamp.now().isoformat(),
        "selected_pairs": SELECTED_PAIRS_5,
        "design": "N_BREAKOUT単独+H1トレンド判定不能除外フィルター(候補①と完全に同一)を5通貨(EUR_USD追加)に適用",
        "train": result, "kpi": kpi,
    }
    out_path = ROOT / "research" / "method-notes" / "candidate1_5pairs_trendfilter_trainonly_backtest.json"

"""EXP-FX000005 外部レビュー対応T-01単独効果測定: 第7試行 + 週末強制クローズ修正のみ.

`backtest_vol_breakout_dow_theory_4pairs_v7_1000usd.py`と検出層・エントリー層・
イグジット層は完全に同一(価格反応型ショック抑制フィルター、TP3=4R)。差分は
`backtest_vol_breakout_dow_theory.py`側の`is_weekend_close_time()`修正
(T-01、外部レビューF1対応)のみが効いた状態を計測するための複製。
T-02(ATRトレール倍率M5再導出)・T-03(決済のM5化)はまだ未適用。
既存のv7結果ファイルは変更せず(T-04での前後比較に必要なため)、別名で出力する。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_v7_t01fix_1000usd_backtest.json
"""
...
def main() -> int:
    print("=== SYS-FX011 改善ループ第7試行(価格反応型ショック抑制フィルター) Train/Validation/Test $1,000バックテスト ===")
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_v7_t01fix_1000usd_backtest.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": (
                "外部レビューT-01(週末強制クローズ修正、F1対応)単独の効果測定。第7試行"
                "(価格反応型ショック抑制フィルター、TP3=4R)と検出層・エントリー層・イグジット層は"
                "完全に同一で、T-01の修正のみが差分。T-02/T-03は未適用。"
            ),

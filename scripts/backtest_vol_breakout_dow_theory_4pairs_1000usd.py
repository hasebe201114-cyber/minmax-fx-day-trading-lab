"""EXP-FX000005 改善ループ第2試行(EUR/USDを除く4通貨版)の
Train/Validation/Test 3期間・$1,000初期資金・確定コストモデルバックテスト.

`backtest_vol_breakout_dow_theory_1000usd.py`(5通貨版、確定コストモデル:
エントリー=成行0.5pip/SL・トレーリング・TP=指値0/週末・MAX_HOLD=成行0.5pip、
25倍レバレッジキャップ込み)と全く同じロジックを、対象通貨をEUR/USDを除く
4通貨(USD/JPY・EUR/JPY・GBP/JPY・AUD/JPY)に絞って再評価する。

パラメータは4通貨版Train(`vol_breakout_dow_theory_4pairs_train.json`)の
導出値をそのまま使用(再学習なし)。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_1000usd_backtest.json
"""
...
def main() -> int:
    print("=== SYS-FX011 ダウ理論連続押し目買い版(4通貨版) Train/Validation/Test $1,000バックテスト ===")
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_1000usd_backtest.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": (
                "改善ループ第2試行: EUR/USDを除く4通貨版。5通貨版と全く同じロジック・確定コストモデル・"
                "25倍レバレッジキャップを適用。パラメータは4通貨Train導出値をそのまま使用(再学習なし)。"
            ),

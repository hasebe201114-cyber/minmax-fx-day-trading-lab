"""EXP-FX000005 外部レビュー対応T-02単独効果測定: 第7試行 + ATRトレール倍率のM5再導出のみ.

外部レビューF3「`atr_trail_multiplier=3.23`はSYS-FX009のH1設計からの転用で、本戦略の
1R(M5スケール、`stop_buffer_atr_m5×ATR(M5)`)に対し3〜4R相当となり、TP3=4Rの全量利確
に先に到達するためトレールが原理的に噛まない」への対応。

事前登録(結果を見る前に固定): トレール幅をATR(M5)基準・1Rの1.0倍(レビュー提案レンジ
0.5〜1.5Rの中央値)で再導出する。すなわち`ATR_TRAIL_MULTIPLIER_M5 = STOP_BUFFER_ATR_M5
(1R定義と同じ係数) × 1.0`とし、H1バーごとにATR(M5)のasof値でトレール幅を計算する
(H1バー単位での判定タイミング自体はT-03で対応するまで変更しない)。

検出層・エントリー層・TP水準(TP3=4R)・価格反応型ショック抑制フィルターは第7試行から
完全に同一。差分はトレール幅の計算方法(ATR系列とtrail_mult)のみ。T-01(週末クローズ
修正)は本測定でも適用済み(共有モジュール`backtest_vol_breakout_dow_theory.py`の修正が
既定で効くため)。T-03は未適用。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_v7_t02fix_1000usd_backtest.json
"""
...
def main() -> int:
    print("=== SYS-FX011 外部レビュー対応T-02単独効果測定(第7試行 + ATRトレールM5再導出) ===")
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_v7_t02fix_1000usd_backtest.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": (
                "外部レビューT-01(週末クローズ)+T-02(ATRトレールM5再導出、F3対応)を"
                "第7試行(価格反応型ショック抑制フィルター、TP3=4R)に累積適用した効果測定。"
                "T-03(決済のM5化)は未適用。"
            ),

"""EXP-FX000017(SYS-FX023) 改善ループ第3試行: 窓内限定でのN_BREAKOUT閾値緩和
(Train単独).

事前登録(`research/EXP-FX000017/00-spec.md`「改善ループ第3試行」節):
BOJ+FOMC窓(候補①)内限定でN_BREAKOUTを緩和する。頻度実測(バックテスト結果を
見ずに算出)から、候補E(N=3.0、窓内64イベント)・候補F(N=2.5、窓内88イベント)
を機械的に選定済み。窓定義・通貨構成は変更しない(1変数の原則)。

出力: research/method-notes/sysfx023_n_relax_trainonly_backtest.json
"""
...
BASELINE_BEST_KPI_COUNT = 5  # ベースライン・第1・第2試行を通じた最良値(いずれも5/9)
...
def main() -> int:
    ...
    print("=== EXP-FX000017(SYS-FX023) 改善ループ第3試行: 窓内N_BREAKOUT閾値緩和 (Train単独) ===\n")
    ...
    print("=== サマリ ===")
    print(f"{'candidate':<20}{'n_trades':>10}{'ペイオフ':>10}{'KPI':>8}{'実効n':>8}{'perm_p':>10}")
    for name in CANDIDATES:
        k = kpis[name]
        print(f"{name:<20}{results[name]['n_trades']:>10}{k['payoff_ratio']:>10}{k['kpi_required_pass_count']:>8}"
    ...
    if selected:
        ...
        print(f"\n[選定] 実効n>=300は未達だが、これまでの最良({BASELINE_BEST_KPI_COUNT}/9)を上回った候補あり。"
              "実効n不足のまま追加検討の材料として記録")
    else:
        print(f"\n[選定] 該当候補なし(実効n>=300未達、これまでの最良{BASELINE_BEST_KPI_COUNT}/9も上回らない) "
              "→ 改善ループ終了、REJECT相当として記録")

    out_path = ROOT / "research" / "method-notes" / "sysfx023_n_relax_trainonly_backtest.json"
    out_path.write_text(json.dumps({
        "generated_at": pd.Timestamp.now().isoformat(),
        "purpose": "EXP-FX000017(SYS-FX023) 改善ループ第3試行: 窓内N_BREAKOUT閾値緩和",

"""EXP-FX000005 改善ループ第5試行: TP3=4R + 初回エントリー除外の組み合わせ.

背景: 司令塔の4点依頼(エントリー/イグジット診断)への回答として、Train単独の
反実仮想スイープで2つの独立した改善候補が判明した:
  - TP3=4Rへの引き上げ: mean_R +13.4%(0.428→0.485)、PF +9.1%(2.142→2.337)
  - entry_seq=1(トレンドイベント内の初回エントリー)の除外: SL率42.8%(2回目
    以降33-39%より明確に高い)という診断結果を受けた候補
司令塔選択「TP3=4Rの組み合わせをValidation確認(推奨) + 初回エントリー除外/
縮小も併せて検証」を受け、両者を組み合わせて改善ループ第3試行(N=3.5・
zigzag=1.0・カレンダー24h+BOJ/FOMC・4通貨)に上乗せしTrain/Validation/Testの
全期間で評価する。HARKing防止: 両候補の選定はTrain単独の反実仮想スイープの
結果のみに基づく(`entry_exit_diagnostics.json`参照)。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_v5_1000usd_backtest.json
"""
...
def find_trades_for_period(pair: str, m5: pd.DataFrame) -> tuple[list[dict], pd.DataFrame, pd.Series]:
    ...
    for i in idxs:
        ...
        trades.extend(simulate_dow_theory_trend(m5, atr_m5, h1, atr_h1, pos, direction,
                                                  ...
                                                  tp_levels=TP_LEVELS_V5, skip_first_entry=True))
...
def main() -> int:
    print("=== SYS-FX011 改善ループ第5試行(TP3=4R + 初回エントリー除外) Train/Validation/Test $1,000バックテスト ===")
    ...
    print(f"事前登録(4通貨Train導出値をそのまま使用): N={N_BREAKOUT}, "
          ...
          f"tp_levels={TP_LEVELS_V5}, skip_first_entry=True, "
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_v5_1000usd_backtest.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": (
                "改善ループ第5試行: 第3試行(4通貨版+カレンダー)にTP3=4R引き上げ+初回エントリー除外を追加。"
                "両候補ともTrain単独の反実仮想スイープで選定(HARKing防止)。"
            ),

"""EXP-FX000005 T-13(出口設計トレール専業化)後の正式KPI判定.

`evaluate_vol_breakout_dow_theory_kpi.py`(ピーク比DD採用済み)の`evaluate_period()`
を`perm_p_field="perm_p_block"`(T-06)・`apply_n_correlation_discount=False`(T-07)
で呼び出し、対象をT-01〜T-09+重複バグ修正+T-13(tp_levels=[]、
breakeven_trigger_r=1.0)の$1,000バックテスト結果に差し替える。

00-spec.md「Test凍結宣言」により、Testの数値もここで算出・記録するが、
修正の妥当性確認のみを目的とし、結論・意思決定にはTrain+Validationのみを用いる。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_v7_trailonly_kpi_evaluation.json
"""
...
def main() -> int:
    print("=== EXP-FX000005 T-13(トレール専業化)後 正式KPI判定 ===\n")
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_v7_trailonly_kpi_evaluation.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": (
                "T-01+T-02+T-03+T-06+T-07+T-09+重複トレード生成バグ修正に加え、"
                "T-13(出口設計をトレール専業に振る: tp_levels=[]、breakeven_trigger_r=1.0)"
                "を適用した正式KPI判定。Test凍結宣言によりTestの数値も算出するが、"

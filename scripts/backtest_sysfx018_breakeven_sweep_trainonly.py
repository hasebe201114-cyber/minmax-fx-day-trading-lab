"""EXP-FX000012(SYS-FX018): breakeven_trigger_rの感度分析(Train評価).

事前登録(`research/EXP-FX000012/00-spec.md`): breakeven_trigger_r∈{0.5,0.75,1.0,
1.5,2.0}の5候補を、現行値(1.0)を中心に均等刻みで機械的に設定する。検出層・
トレンド判定フィルター・stop_buffer_atr_m5・atr_trail_multiplier_m5・
tp_levels=[]・コストモデル・検定方式は完全凍結。

選定ルール: 必須KPI達成数が7/9以上・実効n≥300を維持する候補のうち、
ペイオフレシオが最大の候補を選ぶ(該当なしなら不採用)。

出力: research/method-notes/sysfx018_breakeven_sweep_trainonly_backtest.json
"""
...
BREAKEVEN_CANDIDATES = [0.5, 0.75, 1.0, 1.5, 2.0]  # 事前登録、現行値1.0を中心に均等刻み


def find_trades(pair: str, m5: pd.DataFrame, shock_check, breakeven_trigger_r: float):
    h1 = to_h1(m5)
...
def run_period(breakeven_trigger_r: float, start: str, end: str) -> dict:
    ...
    for pair, m5 in m5_by_pair.items():
        trades = find_trades(pair, m5, shock_check, breakeven_trigger_r)
...
def main() -> int:
    ...
    print(f"=== EXP-FX000012(SYS-FX018): breakeven_trigger_r感度分析 {BREAKEVEN_CANDIDATES} ===\n")

    results = {}
    kpis = {}
    for be in BREAKEVEN_CANDIDATES:
        print(f"--- breakeven_trigger_r={be} ---")
        result = run_period(be, start, end)
        ...
        results[str(be)] = result
        kpis[str(be)] = kpi
        print(f"  トレード数={result['n_trades']}  ペイオフ={kpi['payoff_ratio']}  KPI={kpi['kpi_required_pass_count']}  "
              f"実効n={kpi['n_trades_effective']}  perm_p={kpi['permutation_p_clustered']}\n")

    # 選定ルール: 必須KPI>=7/9かつ実効n>=300を満たす候補のうちペイオフ最大
    eligible = [(be, k) for be, k in kpis.items()
                if k["n_trades_effective"] >= 300
                and int(k["kpi_required_pass_count"].split("/")[0]) >= 7]
    selected = max(eligible, key=lambda x: x[1]["payoff_ratio"]) if eligible else None

    print("=== サマリ ===")
    print(f"{'breakeven_r':<14}{'n_trades':>10}{'ペイオフ':>10}{'KPI':>8}{'実効n':>8}{'perm_p':>10}")
    for be in BREAKEVEN_CANDIDATES:
        k = kpis[str(be)]
        print(f"{be:<14}{results[str(be)]['n_trades']:>10}{k['payoff_ratio']:>10}{k['kpi_required_pass_count']:>8}"
              f"{k['n_trades_effective']:>8}{k['permutation_p_clustered']:>10}")

    if selected:
        print(f"\n[選定] breakeven_trigger_r={selected[0]} (ペイオフ={selected[1]['payoff_ratio']})")
    else:
        print("\n[選定] 該当候補なし(必須KPI>=7/9かつ実効n>=300を同時に満たす候補が存在しない) → 不採用")

    out_path = ROOT / "research" / "method-notes" / "sysfx018_breakeven_sweep_trainonly_backtest.json"
    out_path.write_text(json.dumps({
        "generated_at": pd.Timestamp.now().isoformat(),
        "purpose": "EXP-FX000012(SYS-FX018): breakeven_trigger_rの感度分析(5候補、現行1.0中心に均等刻み)",

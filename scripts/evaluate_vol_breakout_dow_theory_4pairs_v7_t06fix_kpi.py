"""EXP-FX000005 外部レビューT-06(permutation検定のブロック順列化)+T-07(実効n
二重計上の解消)適用後の正式KPI判定.
...
で呼び出し、対象をT-01+T-02+T-03+T-09+T-06+T-07の$1,000バックテスト結果に差し替える。

00-spec.md「Test凍結宣言」により、Testの数値もここで算出・記録するが、
修正の妥当性確認のみを目的とし、結論・意思決定にはTrain+Validationのみを用いる。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_v7_t06fix_kpi_evaluation.json
"""
...
def main() -> int:
    print("=== EXP-FX000005 外部レビューT-06+T-07(検定・n是正)適用後 正式KPI判定 ===\n")
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_v7_t06fix_kpi_evaluation.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": (
                "T-01+T-02+T-03+T-09に加え、外部レビューT-06(permutation検定をブロック順列"
                "(クラスタ=エントリー日)へ差し替え)+T-07(実効n二重計上の解消、"
                "apply_n_correlation_discount=False)を適用した正式KPI判定。"

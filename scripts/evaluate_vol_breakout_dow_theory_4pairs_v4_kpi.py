"""EXP-FX000005 改善ループ第4試行(N=2.5・ZigZag=0.6・カレンダー6h+BOJのみ)の正式KPI判定.

`evaluate_vol_breakout_dow_theory_kpi.py`(ピーク比DD採用済み)と同一のロジックを
再利用し、対象をエントリー機会拡大版の$1,000バックテスト結果に差し替える。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_v4_kpi_evaluation.json
"""
...
def main() -> int:
    print("=== EXP-FX000005 改善ループ第4試行(N=2.5・ZigZag=0.6・カレンダー6h+BOJのみ) 正式KPI判定 ===\n")

    with (ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_v4_1000usd_backtest.json").open(
        encoding="utf-8"
    ) as f:
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_v4_kpi_evaluation.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": "改善ループ第4試行(N=2.5・ZigZag=0.6・カレンダー6h+BOJのみ)。ピーク比DD採用済みの既存版と同一ロジックで判定。",

"""EXP-FX000005 T-14(通貨ペア拡大: EUR_USD追加、5通貨)後の正式KPI判定.

`evaluate_vol_breakout_dow_theory_kpi.py`の`evaluate_period()`を
`perm_p_field="perm_p_block"`(T-06)・`apply_n_correlation_discount=False`(T-07)・
`apply_k3m_scale_invariant=True`(T-08)で呼び出し、対象をT-01〜T-13+T-14
(5通貨版)の$1,000バックテスト結果に差し替える。

00-spec.md「Test凍結宣言」により、Testの数値もここで算出・記録するが、
結論・意思決定にはTrain+Validationのみを用いる。

出力: research/method-notes/vol_breakout_dow_theory_5pairs_trailonly_kpi_evaluation.json
"""
...
def main() -> int:
    print("=== EXP-FX000005 T-14(5通貨、EUR_USD追加)後 正式KPI判定 ===\n")

    with (ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_5pairs_trailonly_1000usd_backtest.json").open(
        encoding="utf-8"
    ) as f:
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_5pairs_trailonly_kpi_evaluation.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": (
                "T-01〜T-09+重複トレード生成バグ修正+T-13(トレール専業化)+T-08(K3m再定義)に、"
                "T-14(通貨ペア拡大: EUR_USDを追加し4通貨→5通貨)を適用した正式KPI判定。"

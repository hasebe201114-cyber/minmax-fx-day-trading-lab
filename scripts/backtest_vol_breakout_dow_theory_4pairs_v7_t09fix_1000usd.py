"""EXP-FX000005 外部レビューT-09対応(=C査読差し戻し最優先): コストモデルのSL・
トレーリング決済にスリッページを復活.

C査読(`research/EXP-FX000005/20-c-review.md`)の差し戻し最優先事項。外部レビュー§2-5
(T-09)「SL・トレーリングも指値注文でよい」という司令塔指示に従った結果、逆指値は
トリガー後は成行と同じ性質で価格保証が無いにもかかわらず、SL_INITIAL_NO_TP・
TP_THEN_SL_TRAIL決済がスリッページ0(スプレッドのみ)で計上されていた。しかも発動する
のは定義上、高ボラティリティ局面(本戦略はまさにその局面を狙う設計)。過去実績では
この1変更だけでTrain+443.8%→+530.2%(約90ポイント)動いている。

事前登録(結果を見る前に固定): SL・トレーリング決済(残存フラクション分)に、外部
レビュー提案レンジ(0.5〜1.0pip)の上限である1.0pipのスリッページを適用する。上限を
採用する理由: 本戦略のSL/トレール決済は構造的に「ボラティリティ・ブレイク検出後の
高ボラ局面」でのみ発生するため(通常時のランダムなSL到達とは異なる)、レンジ内での
恣意的な中間値選択を避け、レビューが提示した範囲の保守的な端点を機械的に採用した。
週末強制クローズ・MAX_HOLD向けの既存スリッページ(0.5pip)とは区別する
(SLIPPAGE_PIPS_STOP_TRIGGERED、新設)。

なお外部レビューが併記した「週末ギャップの別途モデル化」は、T-01で週末保有そのものが
解消された(0/966件、`verify_external_review_findings.py`のF1=NOT_REPRODUCED)ため、
本修正の対象外(適用対象となるポジションが存在しない)。

T-01(週末強制クローズ)・T-02(ATRトレール倍率M5再導出)・T-03(決済判定のM5化)は
このスクリプトでも適用済み(共有モジュールの修正が既定で効くため)。検出層・エントリー
層・TP水準(TP3=4R)・価格反応型ショック抑制フィルターは第7試行から完全に同一。
Test期間は`00-spec.md`「Test凍結宣言」節により参照するが、修正の妥当性確認のみを
目的とし、結論・意思決定には用いない(Train+Validationのみで判断する)。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_v7_t09fix_1000usd_backtest.json
"""
...
def main() -> int:
    print("=== SYS-FX011 外部レビュー対応T-09(コストモデル、C査読差し戻し最優先) ===")
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_v7_t09fix_1000usd_backtest.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": (
                "T-01+T-02+T-03(週末クローズ・ATRトレールM5再導出・決済のM5化)に、"
                "外部レビューT-09(コストモデル、C査読差し戻し最優先)を追加適用。"
                "SL_INITIAL_NO_TP・TP_THEN_SL_TRAILの残存フラクション決済に"
                "1.0pipのスリッページを新規計上(従来はスプレッドのみでスリッページ0)。"

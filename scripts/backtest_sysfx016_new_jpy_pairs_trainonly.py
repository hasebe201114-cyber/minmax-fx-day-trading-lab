"""SYS-FX016(EXP-FX000010) 第1段階: NZD_JPY・CAD_JPY・CHF_JPYを個別に
...
を使用する。EXP-FX000007(SYS-FX013)と同一の方法論・スクリプト構造。

出力: research/method-notes/sysfx016_new_jpy_pairs_trainonly_backtest.json
"""
...
# 00-prescreen.md「コストモデル」節: GMOコイン公式に未掲載のため保守的な暫定値
...
def evaluate_single_pair(pair: str, start: str, end: str) -> dict:
    ...
    shock_check = make_price_shock_check({pair: h1}, {pair: atr_h1})
...
def main() -> int:
    ...
    print("=== SYS-FX016(EXP-FX000010) 第1段階: JPYクロス追加通貨 個別Train評価 ===")
    print("検出層・M5層: SYS-FX012の凍結済み最良候補(候補①+判定不能除外フィルター)をそのまま使用\n")

    results = {}
    for pair in ["NZD_JPY", "CAD_JPY", "CHF_JPY"]:
        print(f"--- {pair} ---")
    ...
    out_path = ROOT / "research" / "method-notes" / "sysfx016_new_jpy_pairs_trainonly_backtest.json"
    out_path.write_text(json.dumps({
        "generated_at": pd.Timestamp.now().isoformat(),
        "design": "SYS-FX012の凍結済み最良候補(候補①=N_BREAKOUT単独+H1トレンド判定不能除外フィルター)を"
                  "NZD_JPY・CAD_JPY・CHF_JPYに個別適用(通貨プールせず単体評価)",
        "note": "コストモデルのスプレッドはGMOコイン公式未掲載のため00-prescreen.md記載の保守的暫定値を使用",
        "results": results,

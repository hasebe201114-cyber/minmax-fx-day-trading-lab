"""EXP-FX000005 外部レビューT-09(コストモデル、C査読差し戻し最優先)適用後の正式KPI判定.

`evaluate_vol_breakout_dow_theory_kpi.py`(ピーク比DD採用済み)と同一のロジックを
再利用し、対象をT-01+T-02+T-03+T-09の$1,000バックテスト結果に差し替える。

00-spec.md「Test凍結宣言」により、Testの数値もここで算出・記録するが、
修正の妥当性確認のみを目的とし、結論・意思決定にはTrain+Validationのみを用いる。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_v7_t09fix_kpi_evaluation.json
"""
...
def main() -> int:
    print("=== EXP-FX000005 外部レビューT-09(コストモデル)適用後 正式KPI判定 ===\n")

    with (ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_v7_t09fix_1000usd_backtest.json").open(
        encoding="utf-8"
    ) as f:
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_v7_t09fix_kpi_evaluation.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": (
                "T-01+T-02+T-03に加え、外部レビューT-09(コストモデル: SL・トレーリング決済への"
                "スリッページ1.0pip復活)を適用した正式KPI判定。Test凍結宣言によりTestの数値も"

"""EXP-FX000005 改善ループ第7試行(価格反応型ショック抑制フィルター)の正式KPI判定.

`evaluate_vol_breakout_dow_theory_kpi.py`(ピーク比DD採用済み)と同一のロジックを
再利用し、対象を改善ループ第7試行の$1,000バックテスト結果に差し替える。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_v7_kpi_evaluation.json
"""
...
def main() -> int:
    print("=== EXP-FX000005 改善ループ第7試行(価格反応型ショック抑制フィルター) 正式KPI判定 ===\n")
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_v7_kpi_evaluation.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": "改善ループ第7試行(価格反応型ショック抑制フィルター、カレンダー固定窓の代わり)。ピーク比DD採用済みの既存版と同一ロジックで判定。",

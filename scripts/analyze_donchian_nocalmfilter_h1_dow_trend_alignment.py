"""SYS-FX012検討: CALM_RATIOフィルターを外した場合(Donchian(20)ブレイクのみ、
N_BREAKOUT重複は除外)のH1ダウ理論順行/逆行分類.

`analyze_donchian_continuation_h1_dow_trend_alignment.py`(CALM_RATIO適用後、
383イベント・1990トレード)との直接比較用。CALM_RATIOという「バーの大きさ」
フィルター自体が、H1トレンド整合性とは独立に質を底上げしているのかを見る。

検出条件: Donchian(20)ブレイク AND NOT(range/ATR>=N_BREAKOUT(3.5))
(CALM_RATIO条件を外した以外はCALM版と完全に同一)
...
def detect_donchian_nofilter_events(h1: pd.DataFrame, atr_h1: pd.Series) -> tuple[list[int], list[str]]:
    dc = donchian(h1["high"], h1["low"], DONCHIAN_LENGTH, DONCHIAN_LENGTH).shift(1)
    ratio = (h1["high"] - h1["low"]) / atr_h1
    close = h1["close"]

    is_spike = ratio >= N_BREAKOUT
    is_cont_up = (close > dc["DCU"]) & (~is_spike)
    is_cont_down = (close < dc["DCL"]) & (~is_spike)
...
def main() -> int:
    print(f"検出対象: Donchian({DONCHIAN_LENGTH})継続 AND NOT(range/ATR>=N_BREAKOUT({N_BREAKOUT}))"
          f"  ※CALM_RATIOフィルターなし")
    ...
    for pair, m5 in m5_by_pair.items():
        ...
        positions, directions = detect_donchian_nofilter_events(h1, atr_h1)
    ...
    print(f"\n=== プール全体(Train、4通貨、CALM_RATIOフィルターなし) ===")
    ...
    result = {
        ...
        "purpose": "CALM_RATIOフィルターを外した場合のDonchian継続分を、H1ダウ理論順行/逆行で分類し、"
                   "CALM_RATIO適用版と比較する",
        "method": {
            "detection": f"Donchian({DONCHIAN_LENGTH})継続 AND NOT(range/ATR>=N_BREAKOUT({N_BREAKOUT}))"
                         f"  ※CALM_RATIOフィルターなし",
        ...
        "comparison_source_with_calm_filter": "research/method-notes/donchian_continuation_h1_dow_trend_alignment.json",
    }
    out_path = ROOT / "research" / "method-notes" / "donchian_nocalmfilter_h1_dow_trend_alignment.json"

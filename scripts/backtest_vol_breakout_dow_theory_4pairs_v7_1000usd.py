...
ショック抑制フィルター(TP3=4Rは第6試行から維持).

司令塔提案「カレンダで固定期間をフィルターするのではなく、ニュースを検知して
エントリー抑制、値動きみて再開判断したほうがよいかも」を受け、
`price_shock_filter.py`で設計した価格反応型フィルター(いずれかの通貨のH1
レンジ/ATR比が7.0に達したら4通貨すべての新規エントリーを抑制、2.0未満が
3本連続したら解除)をBOJ/FOMCカレンダーフィルターの代わりに適用する。
検出層・エントリー層・イグジット層(TP3=4R含む)は第6試行から変更しない。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_v7_1000usd_backtest.json
"""
...
def main() -> int:
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_v7_1000usd_backtest.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": (
                "改善ループ第7試行: カレンダー固定窓(BOJ/FOMC±24h)を価格反応型ショック抑制フィルターに置き換え。"
                "検出層・TP3=4Rは第6試行から維持。"
            ),

"""EXP-FX000005 改善ループ第3試行(4通貨版+経済指標カレンダー)の正式KPI判定.

`evaluate_vol_breakout_dow_theory_kpi.py`(ピーク比DD採用済み)と同一のロジックを
再利用し、対象を4通貨+カレンダーフィルター版の$1,000バックテスト結果に差し替える。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_calendar_kpi_evaluation.json
"""
...
def main() -> int:
    print("=== EXP-FX000005 改善ループ第3試行(4通貨版+経済指標カレンダー) 正式KPI判定 ===\n")
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_calendar_kpi_evaluation.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": "改善ループ第3試行(4通貨版+BOJ/FOMCブラックアウト窓)。ピーク比DD採用済みの5通貨版と同一ロジックで判定。",

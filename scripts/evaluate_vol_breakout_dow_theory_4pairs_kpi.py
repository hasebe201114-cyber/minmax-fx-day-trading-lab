"""EXP-FX000005 改善ループ第2試行(EUR/USDを除く4通貨版)の正式KPI判定.

`evaluate_vol_breakout_dow_theory_kpi.py`(5通貨版、ピーク比DD採用済み)と
同一のロジックをそのまま再利用し、対象を4通貨版の$1,000バックテスト結果に
差し替えて判定する。

出力: research/method-notes/vol_breakout_dow_theory_4pairs_kpi_evaluation.json
"""
...
def main() -> int:
    print("=== EXP-FX000005 改善ループ第2試行(4通貨版) 正式KPI判定 ===\n")

    with (ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_1000usd_backtest.json").open(
        encoding="utf-8"
    ) as f:
        backtest = json.load(f)
    print(f"対象通貨: {backtest['selected_pairs']}\n")
    ...
    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_4pairs_kpi_evaluation.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": "改善ループ第2試行(EUR/USDを除く4通貨版)。ピーク比DD採用済みの5通貨版と同一ロジックで判定。",

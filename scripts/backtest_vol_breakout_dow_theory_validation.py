"""EXP-FX000005 改善ループ第1試行(M5ダウ理論連続押し目買い版)のValidation確認.

背景: Trainでmean_Rがマイナス→プラスに転じたが非有意(p=0.496)だった。本PJは
SYS-FX009(k=0.5タイトストップ)・SYS-FX010(k_stop=1.5)で「Train最良値をそのまま
採用する前に必ずValidationで即座に確認する」という過学習検出の教訓を得ており、
本試行にも同じ規律を適用する。
...
def main() -> int:
    print("=== EXP-FX000005 改善ループ第1試行: M5ダウ理論連続押し目買い Validation確認 ===\n")
    print(f"Train導出パラメータをそのまま使用(再学習禁止): N={N_BREAKOUT}, "
    ...
    print(f"\n=== Train比較 ===  Train mean_R={train_mean_r}  Validation mean_R="
          f"{round(float(np.mean(rs)), 4) if rs else None}")
    if rs and train_mean_r is not None:
        if float(np.mean(rs)) > 0 and train_mean_r > 0:
            ...
            print("符号が反転(Train+→Validation-) — SYS-FX009/010と同型の過学習パターンの可能性")

    out_path = ROOT / "research" / "method-notes" / "vol_breakout_dow_theory_validation.json"
    out_path.write_text(
        json.dumps({
            ...
            "_note": (
                "パラメータはTrain導出値を再学習せずそのまま使用(過学習検出のための"
                "SYS-FX009/010踏襲プロトコル)。この結果はTrainの符号(+0.091)が"
                "Validationで再現するかどうかの判定材料"
            ),
            "trades": all_results,
